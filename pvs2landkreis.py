...
for file in glob.glob(path + '*.csv'):
    print(file)
    pv_helper = pd.read_csv(file, sep = '\t', usecols = [1,2], header = None)
...
pv_all = pv_all.rename(columns = {1: 'PV_coords', 2: 'polygon_coords'})
pv_all = pv_all[pv_all['polygon_coords'] != 'POLYGON ((6.341371801241112 51.82340640781572, 6.341(8.070480264504688, 51.11216465078038, 8.07391834953523, 51.114323022634586)']
...
for i in range(len(liste_key_kreis)):
    try:
        ...
        #duplicate_geometry = helper_df_sjoin["geometry"].centroid.apply(lambda geom: geom.wkb)
        ...
        duplicate_geometry = helper_df_sjoin.geometry
        duplicate_geometry = duplicate_geometry.centroid
        duplicate_geometry = duplicate_geometry.apply(lambda geom: geom.wkb)
        helper_df_sjoin = helper_df_sjoin.loc[duplicate_geometry.drop_duplicates().index]
